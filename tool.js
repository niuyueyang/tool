...
function remove(that) {
	var parents = that.parentNode;
	parents.removeChild(that);
}

//remove其它DOM元素
function removeOthers(element) {
	var elePrrents = element.parentNode;
	elePrrents.removeChild(element);
}
...
function toggle(element) {
	...
	} else if(element.style.display == "block" || !element.style.display) {
...
//实现find,names可以为class,id或者标签tag
function find(names) {
...
function nextAll(element) {
	...
	var arr = [];
	for(var i = _child.length; i > 0; i--) {
		var _childI = _child[i];
		if(_childI == element) {
			break;
		} else if(typeof(_childI) != 'undefined') {
			if(_childI.nodeName != 'SCRIPT') {
				arr.push(_childI);
			}
		}
	}
...
/*注意所有数组操作，只针对一维数组，对象利用JSON.stringify转换后再次使用*/
...
//获取数组共有元素,type为数组元素类型
function sameArr(arr, type) {
	var result = [];
	var obj = {};
	for(var i = 0; i < arr.length; i++) {
...
function sortArr(arr) {
	...
	return sortArr(left).concat(temp, sortArr(right))
}

//判断数组里面是否包含某个元素
function haveArr(arr,item){
	for(var i=0;i<arr.length;i++){
		if(arr[i]==item){
			return true;
		}
...
function classifyArr(arr) {
	var obj = {};
	for(var i = 0; i < arr.length; i++) {
		var firstChar = arr[i].name.substr(0, 1);
		if(!obj[firstChar]) {
			var arrs = [];
			arrs.push(arr[i]);
			obj[firstChar] = arrs;
...
function CC2PY(l1,len) {  
    ...
    if(len!=null||typeof(len)!='undefined'){
    	return I1.substr(0,len);  
    }else{
    	return I1.substr(0,lens);  
    }
}  
// 在对象中搜索 
...
//var time=beforeNowtime(19168);
//time=time.replace(/-/g,':').replace(' ',':');
//time=time.split(':');
//var time1 = new Date(time[0],(time[1]-1),time[2],time[3],time[4],time[5]);
//var beforeTime=Date.parse(time1);
